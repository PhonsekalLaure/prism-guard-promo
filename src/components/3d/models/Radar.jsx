...
function SweepFan() {
  ...
  // Pie slice geometry for radar sweep
  const geo = useMemo(() => {
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);
    const angle = Math.PI / 3; // 60° sweep
    const r = 1.8;
...
export default function Radar() {
  ...
  useFrame((state, delta) => {
    // Slight wobble
    dishRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.5) * 0.05;
    ring1Ref.current.rotation.z += delta * 0.25;
    ring2Ref.current.rotation.z -= delta * 0.15;
  });

  return (
    <group scale={1.5} ref={dishRef}>
      {/* Base plate */}
      ...
      <group ref={ring1Ref} rotation={[Math.PI / 2, 0, 0]} position={[0, 0, 0]}>
        <mesh>
      ...
      {/* Ping pulses at random positions on the dish */}
      {[
        [[0.8, 0.06, 0.6], 0.0],
        [[-0.5, 0.06, 1.1], 0.8],
        [[1.1, 0.06, -0.7], 1.5],
