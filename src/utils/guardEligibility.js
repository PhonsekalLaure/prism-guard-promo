...
export function getApplicantBirthDateBounds({ minAge = MIN_GUARD_AGE, maxAge = MAX_GUARD_AGE } = {}) {
  const todayDate = new Date();
  const oldestAllowedBirthDate = new Date(
    todayDate.getFullYear() - maxAge - 1,
    todayDate.getMonth(),
    todayDate.getDate() + 1
  );
  ...
  return {
    ...
    max: toDateInputValue(new Date(todayDate.getFullYear() - minAge, todayDate.getMonth(), todayDate.getDate())),
  };
}
